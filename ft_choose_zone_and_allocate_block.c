...
int	ft_choose_zone(size_t nbr_of_bytes)
{
	...
	if (nbr_of_bytes < 0)
        {
		ft_handle_negative_size();
		return (zone); 
	}
        else if (nbr_of_bytes >= 0 && nbr_of_bytes <= TINY)
        	return (0);     
...
{
	//This function find the choosen block then split the block to two blocks
	//if the block has more the required bytes and the remain bytes
	//can fit new block minimum size (sizeof(t_block stuct) + 1 bytes)
	t_block		*temp_block;
	t_block		*new_block;
	...
	while (temp_block != NULL)
	{
		...
		{
			if (temp_block.size - nbr_of_bytes >= sizeof(t_block) + 1)
			{
				new_block = (t_block *) ((char *)temp_block + sizeof(t_block) + nbr_of_bytes);
				new_block->next = temp_block->next;
				temp_block->next = new_block;
				new_block->size = temp_block->size - nbr_of_bytes - sizeof(t_block);
				new_block->free = 1;
				temp_block->size = nbr_of_bytes;
				temp_block->free = 0;
